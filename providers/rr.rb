...
def load_current_resource
  ...
  @dyn = Dyn::Traffic::Client.new(@new_resource.customer, @new_resource.username, @new_resource.password, @new_resource.zone)
  ...
  begin
    @rr = Dyn::Traffic::Resource.new(@dyn, @new_resource.record_type, @new_resource.zone).get(@new_resource.fqdn)
    ...
    Chef::Log.debug("Cannot find resource #{@new_resource} at dyn")
  end
end

def action_create
  unless @rr
    rr = Dyn::Traffic::Resource.new(@dyn, @new_resource.record_type, @new_resource.zone)
    rr.fqdn(@new_resource.fqdn)
    rr.ttl(@new_resource.ttl) if @new_resource.ttl
    rr.rdata = @new_resource.rdata
    rr.save
    @dyn.publish
    Chef::Log.info("Added #{@new_resource} to dyn")
...
def action_update
  if @rr
    ...
    if changed
      @rr.save
      @dyn.publish
      Chef::Log.info("Updated #{@new_resource} at dyn")
      new_resource.updated_by_last_action(true)
    end
  else
    action_create
...
def action_replace
  ...
  rr.save(true)
  @dyn.publish
  Chef::Log.info("Replaced #{@new_resource} at dyn")
  new_resource.updated_by_last_action(true)
end
...
def action_delete
  if @rr
    @rr.delete
    @dyn.publish
    Chef::Log.info("Deleted #{@new_resource} from dyn")
    new_resource.updated_by_last_action(true)
  end
end
